...
프로젝트의 본 진행 과정에서는 under sampling을 통해 재직자의 데이터 수를 10,000건 만 추출해서 사용하였다. 하지만 일반적으로 딥러닝 학습 과정에는 수 천에서 수 만 건의 데이터가 사용된다. 따라서 over sampling 기법인 SMOTE을 적용하여, 퇴직자 데이터 수를 9만 개로 증폭시켜 추가적인 모델링을 진행했다. 

모델의 구조는 다음과 같다.

1. 기본적인 DeepMLP 구조를 따름.
2. ReLU 대신 LeakyReLU를 사용하여 0 이하의 입력 값은 모두 0으로 처리하는 Dying ReLU 문제를 방지.
3. Batch Normalization을 매 층 적용하여 학습을 안정화.
...
|:-------------|:-----------|:--------|:----------|:---------|
| 재직자(0)    | 0.83      | 0.79   | 0.81     | 17,998  |
| 퇴사자(1)    | 0.80      | 0.84   | 0.82     | 17,998  |
| Accuracy |      -     |   -     | 0.81 | 35,996  |
| Macro Avg | 0.81    | 0.81   | 0.81     | 35,996  |
| Weighted Avg | 0.81 | 0.81   | 0.81     | 35,996  |
...
<br>**이용채**<br>
이론적인 학습에서 벗어나 실제로 모델을 구축하고 응용할 수 있는 프로젝트였습니다. 단순히 복잡한 모델을 설계하고 코딩하는 것이 아니라, 데이터의 특성을 파악하고 이를 고려해 모델을 최적화하는 총체적인 과정을 경험할 수 있었습니다. 무엇보다 이 과정을 다 함께 고민하며 즐겁게 진행할 수 있게 해준 팀원들께 샤라웃..   <br>


<br>**김나은**<br>
이번 프로젝트를 통해  단순히 성능이 좋은 모델을 찾는데 집중하기보다 주어진 데이터의 특성을 고려해 다양한 접근 방식으로 문제에 맞는 적절한 모델을 선택하는 것이 중요하다는 것을 배웠습니다.<br>


<br>**임종민**<br>
이번 프로젝트는 주어진 데이터에 가장 적합한 학습 모델을 탐색하는 과정이었습니다. 머신러닝과 딥러닝을 실제로 적용해보며, 정답이 분명하지 않은 상황 속에서도 방향을 함께 고민해준 팀원들에게 감사드립니다.<br>


<br>**차하경**<br>
프로젝트 초반에는 예측 성능이 낮아 어려움을 겪었지만, 가중치를 적용해 수치형 변수의 분포를 조정하며 성능 개선을 할 수 있었습니다. 이후 샘플링 전략과 딥러닝 구조 최적화를 거쳐 예측 성능을 점차 끌어올렸고 Optuna 기반 튜닝을 반복하면서 파라미터 조정이 결과에 미치는 영향을 체감할 수 있었고, 데이터 기반 문제 해결의 과정을 실감할 수 있던 값진 경험이었습니다.<br>


<br>**김민석**<br>
이번 프로젝트를 하면서 그동안 머신러닝과 딥러닝의 차이를 구분 안하고 있었는데 확실히 무엇이 다른지 알게되었고 데이터에 따라서 어떤 모델과 하이퍼파라미터 값들을 사용 해야 하는지 알게 되는 계기가 되었습니다. 또한 적합한 데이터를 찾아내는것도 얼마나 힘든 과정인지 깨달았습니다.<br>
