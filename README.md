...
Target인 Resigned 변수는 퇴직자(1)에 비해 재직자(0)의 비율이 약 9:1로 클래스가 매우 불균형한 데이터입니다. 이러한 경우 많은 모델이 데이터의 대부분을 차지하는 다수 클래스에 치우친 예측을 하기 때문에 정확도는 높아보일지 몰라도 의미 있는 예측을 할 수 없습니다. 하지만 이탈 예측 모델 같은 경우 소수 클래스인 퇴사를 분류하는 것이 더 중요하기 때문에 불균형을 해소할 필요가 있습니다.  
