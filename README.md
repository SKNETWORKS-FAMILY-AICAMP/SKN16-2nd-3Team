...
<추가>
 딥러닝을 하기 위해 많은 데이터가 필요하다. 따라서 샘플링을 해야 하는데 True 값이 False 값보다 현저히 적엇 True 값을 오버 샘플링을 했다. SMOTE 오버샘플링을 적용하여 True 값의 데이터를 약9만개로 증폭시켜서 True 값과 False 값을 9만개가 되어그래서 약 18만개의 데이터를 가지고 딥러닝을 하였다.
 다음으로 활성화 함수는 ReLU 대신 LeakyReLU를 사용해서 0이하의 값은 모두 0으로 처리하는 dying ReLU 문제를 방지했다. 그리고 Batch Normalization을 매 층마다 적용해서 학습 안정화하였다.

 출력층에는 Sigmoid를 쓰지 않았고 대신에
손실 함수로 nn.BCEWithLogitsLoss() 사용했다.(("비씨이 위드 로짓스 로스" (영어: Binary Cross Entropy With Logits Loss))((이 함수는 내부적으로 Sigmoid를 자동으로 적용하고, Binary Cross Entropy를 계산함.))

따라서 출력층에 따로 Sigmoid를 넣으면 중복 적용이 되기 때문에 쓰지 않았다. optimizer 는 Adam 사용했다.
